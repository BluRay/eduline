...
class quickpay_service
{
    ...
    {
            //读取银联配置
            $unionpay = M('system_data')->where("`list`='admin_Config' AND `key`='unionpay'")->field('value')->find();
            $uData = @unserialize($unionpay['value']);
            quickpay_conf::$pay_params['merId']         = $uData['unionpay_merId'];
            quickpay_conf::$pay_params['merAbbr']       = $uData['unionpay_merAbbr'];
            quickpay_conf::$security_key = $uData['unionpay_key'];
        $param_check    = array();
        switch($service)
        {
        ...
        //signature
        $this->args['signature']    = self::sign($this->args, quickpay_conf::$sign_method,quickpay_conf::$security_key);
    ...
    static function sign($params, $sign_method,$keys)
    {
