...
function rcalendar(text, mode, retfunction) { //文本域对象, 模式(dateonly,full), 选择日期后的事件函数(函数是新时间的Date对象)
	...
	if (!div_rcalendar) { //如果不存在，则初始化创建它
		...
		if (document.compatMode == "BackCompat" && navigator.userAgent.indexOf("MSIE") != -1) {
			...
			css += "#rcalendar {width:190px; height:180px;  background:#FFFFFF; font-size:12px; font-family:宋体; border:1px solid #999999; padding:4px; overflow:hidden;}";
			css += "#rcalendar_ym {width:190px; height:16px; overflow:hidden; margin-bottom:4px;}";
			css += "#rcalendar_y {width:48px; float:left; font-weight:bold; color:#777777; padding-left:2px;}";
			css += "#rcalendar_m {width:50px; float:left; font-weight:bold; color:#777777;}";
			css += "#rweeks {width:190px; height:25px; overflow:hidden;}";
			css += "#rdates {width:190px; height:110px; overflow:hidden;}";
			css += ".rweek {width:27px; height:15px; padding:4px 0px 4px 0px; float:left;text-align:center; background:#CCCCCC; color:#FFFFFF; overflow:hidden;}";
			...
			css += "#rhours {width:104px; height:150px; overflow:hidden;margin-top:-25px;}";
