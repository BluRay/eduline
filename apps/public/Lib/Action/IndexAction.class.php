...
class IndexAction extends Action {
	...
	public function showFaceCard() {
		...
		$data['department'] = isset($depart[$data['userInfo']['department_id']]) ?
        $depart[$data['userInfo']['department_id']] : '';
		//查询关注
        $count['following_count']=M('UserFollow')->where(array('uid'=>intval($_REQUEST['uid'])))->count();
        //查询粉丝
        $count['follower_count']=M('UserFollow')->where(array('fid'=>intval($_REQUEST['uid'])))->count();
        //查询课程数量
        $count['video_count']=D("ZyOrder")->where(array('uid'=>intval($_REQUEST['uid']),'is_del'=>0))->count();
        //查询问答数量
        $count['wenda_count']=M('ZyWenda')->where(array('uid'=>intval($_REQUEST['uid']),'is_del'=>0))->count();//加载我的问答数量
