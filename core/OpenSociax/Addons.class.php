...
class Addons
{
    ...
    public static function addonsHook($addonsName, $name, $param = array(), $admin = false)
    {
        if(!$addonsName)
            return;
        $addonsName = basename($addonsName);        //  (用户登录为例)Login
        $dirName = ADDON_PATH.'/plugin';            //  E:/www/..../addons/plugin
        $urlDir = SITE_URL.'/addons/plugin';        //  www.jiaoyu.com/addons/plugin
        $path = $dirName.'/'.$addonsName;           //  addons/plugin/Login
        $addonUrl = $urlDir.'/'.$addonsName;        //  www.jiaoyu.com/addons/plugin/Login
